/**
 * Strengths/weaknesses/organizational-fit text generators — ported from the
 * webapp's src/app/utils/insights.ts (generateKolbInsights,
 * generateSternbergInsights, generateDualProcessInsights). This is the
 * live version the webapp actually renders (AssessmentReport.tsx,
 * ProfessionalAssessmentReport.tsx) — assessmentInsights.ts is dead code
 * in the webapp and was not ported.
 *
 * Adaptation: the webapp's `scores` params are typed to strict per-axis
 * shapes (e.g. {CE,RO,AC,AE}) on the raw Likert-sum scale, used only for a
 * single "are dimensions unbalanced" spread check. Mobile's
 * src/utils/scoring.ts produces Record<styleName, 0-100> composite scores
 * instead, so that check is generalized to a plain max-min spread with a
 * threshold re-tuned for the 0-100 scale (25, chosen to flag a clearly
 * lopsided profile without over-triggering on ordinary variation).
 */
...
const UNBALANCED_SPREAD_THRESHOLD = 25; // 0-100 scale

function spread(scores: Record<string, number>): number {
  const values = Object.values(scores);
  if (values.length === 0) return 0;
  return Math.max(...values) - Math.min(...values);
}

export function generateKolbInsights(style: string, scores: Record<string, number>): AssessmentInsights {
  ...
  if (spread(scores) > UNBALANCED_SPREAD_THRESHOLD) {
...
export function generateSternbergInsights(style: string, scores: Record<string, number>): AssessmentInsights {
  const insights: AssessmentInsights = {
    strengths: [],
    weaknesses: [],
    improvements: [],
...
export function generateDualProcessInsights(style: string, scores: Record<string, number>): AssessmentInsights {
  const insights: AssessmentInsights = {
    strengths: [],
    weaknesses: [],
    improvements: [],
...
/** Dispatcher for mobile's known assessment types. */
export function getStyleInsights(
  assessmentType: 'learning' | 'thinking' | 'decision',
  primaryStyle: string,
  scores: Record<string, number>
): AssessmentInsights {
  if (assessmentType === 'learning') return generateKolbInsights(primaryStyle, scores);
  if (assessmentType === 'thinking') return generateSternbergInsights(primaryStyle, scores);
  return generateDualProcessInsights(primaryStyle, scores);
}
