/**
 * Professional Cognitive Assessment scoring — verbatim port of the webapp's
 * src/app/utils/professionalCognitiveScoring.ts. No RN imports; pure logic.
 */

// Mirrors the shape webapp's ProfessionalCognitiveAssessment.tsx builds from
// its response arrays (originally imported as a type from that component).
...
function calculateMatchScore(learningScore: number, thinkingScore: number, decisionScore: number): number {
  ...
  // Final score (floored at 50, capped at 98 for professional realism)
